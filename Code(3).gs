 ...
 *  Version: 3.12 |  Updated: 2026-04-15
 * ═══════════════════════════════════════════════════════════════════
 *
 *  SETUP STEPS:
 ...
 *
 *  CHANGELOG v3.12:
 *  - FIX: checkInDate and checkOutDate now stored in sheet (columns T + U).
 *         TOTAL_COLS expanded 19→21. _logToSheet signature extended.
 *         _handleSubmit reads both fields from payload/formData.
 *         migrateSheetV32() handles existing 19-col sheets: adds cols T+U.
 *  - FIX: Stale 'v3.6' version string in success Logger.log → v3.12.
 *  - FIX: _purgeOldSubmissionIds() now also called from doPost (was only doGet).
 *  - FIX: testSubmit() now includes submissionId + checkInDate/checkOutDate
 *         so the dedup code path is exercised on every test run.
 *
 *  CHANGELOG v3.11:
  - NEW: _handleSubmit() deduplicates on submissionId (UUID from front-end).
         PropertiesService stores recent submission IDs for 24h.
         Duplicate POSTs return { status:"success", duplicate:true } so the
         front-end succeeds silently without writing a second sheet row.
         Fixes fetchWithRetry() double-row risk on 3G/cold-start retries.

  CHANGELOG v3.10:
 *  - FIX (BUG-4): Replaced 3 stale 'v3.5' version strings with 'v3.6'
 *    in doGet response, _handleSubmit log, and success log.
 *  - No schema or payload changes. index.html bumped to v3.10 (photo system).
 *
 *  CHANGELOG v3.6:
...
//  S  Photos Folder   T  Check-In Date   U  Check-Out Date
const COL = {
  TIMESTAMP:  1,
  DATE:       2,
  PROPERTY:   3,
...
const TOTAL_COLS = 21; // v3.12: was 19; +2 for checkInDate, checkOutDate
// ──────────────────────────────────────────────────────────────────
...
//  ROUTER
// ═══════════════════════════════════════════════════════════════════
// ── v3.11: Purge submission ID dedup keys older than 24h ───────────────────
// Called on every GET request (cheap, runs rarely in field use).
function _purgeOldSubmissionIds() {
  try {
    ...
    for (const key in all) {
      if (!key.startsWith('seen_sid_')) continue;
      const ts = new Date(all[key]).getTime();
      if (!isNaN(ts) && ts < cutoff) toDelete.push(key);
    }
    toDelete.forEach(k => props.deleteProperty(k));
    if (toDelete.length > 0) Logger.log('Purged ' + toDelete.length + ' stale submissionId(s)');
  } catch(e) { /* non-fatal */ }
}

function doGet(e) {
  _purgeOldSubmissionIds(); // v3.11: cleanup old dedup keys
  ...
  return _json({ ok: true, msg: 'Cascade Hideaway Web App v3.12 — online' });
}

function doPost(e) {
  _purgeOldSubmissionIds(); // v3.12: also purge stale dedup keys on POST (was doGet-only)
  try {
...
function _handleSubmit(payload) {
  ...
  // ── v3.11: Deduplication on submissionId ────────────────────
  // Front-end generates a UUID per submission attempt. On a 3G retry
  // the same UUID is reused. If we've already processed it, return
  // success immediately without writing a second row.
  const submissionId = (payload.submissionId || '').toString().trim();
  if (submissionId) {
    const props = PropertiesService.getScriptProperties();
    const PROP  = 'seen_sid_' + submissionId;
    if (props.getProperty(PROP)) {
      Logger.log('DUPLICATE submission detected — id: ' + submissionId + ' — skipping write');
      return _json({ status: 'success', duplicate: true, message: 'Duplicate submission ignored.' });
    }
    // Mark as seen — auto-expires after 24h via a cleanup approach
    // (PropertiesService has no TTL; we store a timestamp and prune on GET)
    props.setProperty(PROP, new Date().toISOString());
  }
  ...
  // ── v3.12: Guest check-in / check-out dates ─────────────────
  const checkInDate  = String(payload.checkInDate  || formData.checkInDate  || '—').trim() || '—';
  const checkOutDate = String(payload.checkOutDate || formData.checkOutDate || '—').trim() || '—';

  Logger.log('=== CLEANING REPORT RECEIVED (v3.12) ===');
  ...
  _logToSheet(
    ...
    reportFolder.getUrl(),
    checkInDate, checkOutDate  // v3.12
  );
  ...
  Logger.log('=== REPORT PROCESSED SUCCESSFULLY (v3.12) ===');
  ...
  try {
    ...
    if (!sheet) {
      ...
      sheet.getRange(1, 1, 1, TOTAL_COLS).setValues([[
        ...
        'Check-In Date',  'Check-Out Date'  // v3.12
      ]]);
    ...
    sheet.getRange(newRow, 1, 1, TOTAL_COLS).setValues([[
      ...
      checkInDate  || '—',                               // T  Check-In Date  (v3.12)
      checkOutDate || '—'                                // U  Check-Out Date (v3.12)
    ]]);
...
function migrateSheetV32() {
  ...
  if (colCount >= TOTAL_COLS) {
    Logger.log('Already at v3.12+ width (' + TOTAL_COLS + ' cols). Refreshing Monthly Summary only.');
    _updateMonthlySummary(ss);
    return;
  }

  // ── v3.12: 19-col sheet → add Check-In Date (T) + Check-Out Date (U) ──
  if (colCount === 19) {
    Logger.log('v3.11 schema (19 cols). Adding Check-In Date + Check-Out Date columns.');
    sheet.insertColumnAfter(19);
    sheet.insertColumnAfter(20);
    sheet.getRange(1, COL.CHECK_IN ).setValue('Check-In Date');
    sheet.getRange(1, COL.CHECK_OUT).setValue('Check-Out Date');
    const dataRows = sheet.getLastRow() - 1;
    if (dataRows > 0) {
      sheet.getRange(2, COL.CHECK_IN,  dataRows, 1).setValue('—');
      sheet.getRange(2, COL.CHECK_OUT, dataRows, 1).setValue('—');
    }
    sheet.getRange(1, 1, 1, TOTAL_COLS)
         .setFontWeight('bold').setBackground('#22333B').setFontColor('#FFFFFF');
    try { sheet.autoResizeColumns(1, TOTAL_COLS); } catch(e) {}
    _updateMonthlySummary(ss);
    Logger.log('✅ v3.12 migration complete. Deploy a NEW VERSION of the script.');
    return;
  }
...
function testSubmit() {
  ...
  // v3.12: include submissionId so the dedup code path is exercised;
  //        include checkInDate/checkOutDate to validate new schema columns.
  const testId = 'test-' + today + '-' + Math.random().toString(36).slice(2, 8);
  const result = _handleSubmit({
    ...
    deltaM3:                  1.4,
    cleaningDate:             today,
    checkInDate:              today,                 // v3.12
    checkOutDate:             today,                 // v3.12
    completionRate:           100,
  ...
  Logger.log('✅ submissionId used: ' + testId);
  Logger.log('✅ Check sheet (cols T+U for check-in/out) and email inbox for the test row.');
}
